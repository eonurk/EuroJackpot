...
ggplot(results, aes(x = year, fill = weekday)) +
    geom_bar(width = 0.72) +
    scale_fill_manual(values = c("Friday" = ej_blue, "Tuesday" = ej_green), name = NULL) +
    labs(
...
ggplot(euro_df, aes(x = number, y = count, fill = type)) +
    ...
    scale_fill_manual(values = c("Observed" = ej_gold, "Expected (exposure-adjusted)" = ej_muted), name = NULL) +
    labs(
        ...
        x = "Euro number", y = "Appearances", caption = ej_caption
    ) +
...
ggplot(main_df, aes(x = number, y = count)) +
    ...
    geom_hline(yintercept = expected_main, color = ej_ink, linewidth = 0.55) +
    geom_hline(yintercept = c(expected_main - band, expected_main + band), linetype = "dashed", color = ej_muted) +
...
ggplot() +
    geom_col(data = gap_counts, aes(x = gaps, y = Freq), fill = ej_blue, width = 0.8) +
    geom_line(data = geo, aes(x = gap, y = expected), color = ej_ink, linewidth = 1) +
    labs(
        title = "Waiting Times Between a Number's Appearances vs the Memoryless Ideal",
        subtitle = sprintf(
            "%d observed gaps, mean %.1f draws (theory: 10.0) - the dark curve is the geometric distribution",
...
<div class="pull-stat"><span class="big">`r sprintf("%.2f%%", 100 * overdue_hits / overdue_trials)` vs 10.00%</span><span class="why">Hit rate of "overdue" numbers on the very next draw, tested across the entire history. Being due is not a thing.</span></div>
...
ggplot(rep_df, aes(x = k, y = count, fill = type)) +
    geom_col(position = "dodge", width = 0.72) +
    scale_fill_manual(values = c("Observed" = ej_blue, "Expected" = ej_muted), name = NULL) +
    labs(
...
ggplot(bt_plot, aes(x = strategy, y = mean)) +
    ...
    geom_errorbar(aes(ymin = mean - ci, ymax = mean + ci), width = 0.22, color = ej_ink, linewidth = 0.55) +
    geom_hline(yintercept = 0.5, color = ej_red, linetype = "dashed", linewidth = 0.9) +
    labs(
        ...
        x = NULL, y = "Mean matches per draw", caption = ej_caption
    ) +
    theme(
        panel.grid.major.x = element_blank(),
...
ggplot(rollover, aes(x = tickets / 1e6, y = p_rollover, color = era)) +
    ...
    scale_color_manual(values = ej_ramp(3), name = NULL) +
    labs(
        ...
        x = "Tickets sold (millions)", y = "P(no jackpot winner)", caption = ej_caption
    ) +
